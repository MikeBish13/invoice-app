...
export default function InvoicePage(props) {
    ...
    const {data, status} = useQuery(['invoice', invoiceId], () => fetchInvoice(invoiceId))
    ...
    return (
        ...
        {status === 'success' && (
            ...
            <div>
                ...
                <button onClick={() => setPaidInvoice(data[0].id)}>Mark as paid</button>
                </div>
                <h2>#{data[0].id}</h2>
                ...
                <br></br>
                {data[0].items && data[0].items.map(item => 
                        <div key={item.id}>
                            <p>{item.name}</p>
                            <p>{item.quantity} x £{formatCost(item.price)}</p>
                            <p><strong>£{formatCost(item.total)}</strong></p>
                        </div>
                )}
